    ...
    def numberOfGoodPaths(self, vals: List[int], edges: List[List[int]]) -> int:
        ...
        for val in sorted(same_val_nodes.keys()):   # {1: {0, 3}, 2: {2}, 3: {1, 4}}
            for i in same_val_nodes[val]:    # {0, 3}  {2}  {1, 4}
                for nb in adj[i]:            # {0: {1, 2}, 1: {0}, 2: {0, 3, 4}, 3: {2}, 4: {2}}
                    if vals[nb] <= val:      # Only choose neighbors with a smaller value or equal to the starting node
                        union(nb, i)        

            cnts = collections.Counter()
            for i in same_val_nodes[val]:  # {0, 3}  {2}  {1, 4}
                cnts[find(i)] += 1         # {0: 1} {0: 1, 3: 1}
